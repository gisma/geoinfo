---
title: "Tutorial: Analyzing massive amounts with R, gdalcubes, and STAC"
author: "Marius Appel adapted by Chris Reudenbach"
date: "Sept. 1, 2021"
...
if (!file.exists(paste0(tutorialDir,"gemeinden.zip")))
  downloader::download(url ="https://daten.gdz.bkg.bund.de/produkte/vg/vg250-ew_ebenen_1231/aktuell/vg250-ew_12-31.tm32.shape.ebenen.zip", destfile = paste0(tutorialDir,"gemeinden.zip"))
# Entpackt werden nur die Gemeindegeometrien
unzip(zipfile = paste0(tutorialDir,"gemeinden.zip"),
      ...
      exdir = paste0(tutorialDir,"gemeinden/"),
      junkpaths = TRUE)
gemeinden_sf = st_read(paste0(tutorialDir,"gemeinden/VG250_GEM.shp"))
...
```{r geometries}
st_crs(hessen)
hessen = st_transform(hessen,crs = 3035)
...
lahntal = gemeinden_sf_3035 %>% filter(GEN == "Lahntal")
tmap_mode("view")
tm_shape(st_as_sfc(bbox)) + tm_polygons()  + tm_shape(lahntal) +   tm_polygons()
```
...
s2_collection = stac_image_collection(items$features, asset_names = assets, property_filter = function(x) {x[["eo:cloud_cover"]] < 5})
s2_collection
```

As a result we get an image collection with 46 images and the `SCL` band.
...
```{r}
v.hessen.overview = cube_view(srs="EPSG:32632",  dx=200, dy=200, dt="P30D", 
...
```{r lahntal}
 lahntal_32632 = st_transform(lahntal,crs = 32632)
 v.lahntal = cube_view(view = v.hessen.overview, dx=10, dy=10,
...
```{r}
library(sf)
lahn = st_transform(lahntal,crs = 4326)
```
...
v = cube_view(srs = "EPSG:32633", extent = list(t0 = "2016-01",t1 = "2021-12", 
...
if (file.exists(paste0(tutorialDir,"lahntal.nc")))
  file.remove(paste0(tutorialDir,"lahntal.nc"))
```

```{r}
s2.mask = image_mask("SCL", values = c(3,8,9))
gdalcubes_options(threads = 30, ncdf_compression_level = 5)
...
if (file.exists(paste0(tutorialDir,"bf_results.nc"))) 
  file.remove(paste0(tutorialDir,"bf_results.nc"))
```
...
```{r bfast_change, out.width="100%"}
library(stars)
ncdf_cube(paste0(tutorialDir,"bf_results.nc")) |>
  st_as_stars() -> x
tm_shape(x['NETCDF:"bfresult1.nc":change_date']) + tm_raster() + tm_shape(x['NETCDF:"bfresult1.nc":change_magnitude'])  + tm_raster()
